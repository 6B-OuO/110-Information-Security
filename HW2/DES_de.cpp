...
int main(int argc, char* argv[])
{
	...
	string input = argv[2];
	string key1 = argv[4];
	//cout << input.length() << " " << key1.length();
	int L = input.length();
	if (L != 18) {
		string sub = input.substr(2, L-2);
		//cout << sub;
		input = "";
		...
		input = input.substr(2, L - 2);
	}

	L = key1.length();
	if (L != 18) {
		string sub = key1.substr(2, L - 2);
		//cout << sub;
		key1 = "";
		...
		key1 = key1.substr(2, L - 2);
	}
	//cout << input << endl << key1 ;
	// pt is plain text
	string pt, key;
	...
	string text = encrypt(input, rkb, rk);
	text = "0x" + text;
	//cout << "\nPlain Text: ";
	cout << text << endl;
}
